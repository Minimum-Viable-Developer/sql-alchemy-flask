...
class GenusSchema(SQLAlchemyAutoSchema):
    class Meta:
        model = Genus
        include_relationships = True
        load_instance = True

class SpeciesSchema(SQLAlchemyAutoSchema):
    class Meta:
        model = Species
        include_relationships = True
        load_instance = True

class SpecimenSchema(SQLAlchemyAutoSchema):
    class Meta:
        model = Specimen
        include_relationships = True
        load_instance = True
...
@app.route('/specimens_marshmellow', methods=['GET'])
def get_specimens_marshmellow():
    session = db_connect()
    specimen_schema = SpecimenSchema()
    specimens = session.query(Specimen).all()
    output_list = []
    for specimen in specimens:
        output_list.append(specimen_schema.dump(specimen))
